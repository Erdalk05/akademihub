  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border border-gray-200 rounded-xl overflow-hidden">
        <div className="bg-amber-50 p-3 flex items-center gap-2 border-b">
          <Settings size={16} className="text-amber-600" />
          <h3 className="font-semibold text-sm">Özel Şablon Tanımla</h3>
        </div>
        ...
        <div className="p-4 grid grid-cols-2 gap-3">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-xs font-medium text-gray-700 mb-1 block">Kurum Adı</label>
              <input 
                type="text" 
                className="w-full px-2 py-1.5 border rounded text-sm focus:ring-2 focus:ring-amber-500" 
                value={ozelSablonForm.kurumAdi}
                onChange={(e) => setOzelSablonForm({...ozelSablonForm, kurumAdi: e.target.value})} 
              />
            </div>
            
            <div>
              <label className="text-xs font-medium text-gray-700 mb-1 block">Sınıf</label>
              <input 
                type="text" 
                className="w-full px-2 py-1.5 border rounded text-sm focus:ring-2 focus:ring-amber-500"
                value={ozelSablonForm.sinif}
                onChange={(e) => setOzelSablonForm({...ozelSablonForm, sinif: e.target.value})} 
              />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-medium text-gray-700">Alan Seçenekleri</label>
              {[
                { key: 'tcKimlik', label: 'T.C. Kimlik No' },
                { key: 'adSoyad', label: 'Ad Soyad' },
                { key: 'cinsiyet', label: 'Cinsiyet' },
                { key: 'kitapcikTuru', label: 'Kitapçık Türü' }
              ].map(alan => (
                <label key={alan.key} className="flex items-center gap-2 text-sm cursor-pointer">
                  <input 
                    type="checkbox" 
                    className="w-4 h-4 rounded"
                    checked={ozelSablonForm[alan.key as keyof typeof ozelSablonForm] as boolean}
                    onChange={(e) => setOzelSablonForm({...ozelSablonForm, [alan.key]: e.target.checked})} 
                  />
                  <span>{alan.label}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-medium text-gray-700">Dersler</label>
              <button 
                className="text-xs px-2 py-1 bg-amber-500 text-white rounded hover:bg-amber-600 flex items-center gap-1"
                onClick={() => setOzelSablonForm({...ozelSablonForm, dersler: [...ozelSablonForm.dersler, {dersKodu: '', dersAdi: '', soruSayisi: 0}]})}>
                <Plus size={12} />
                Ders Ekle
              </button>
            </div>
            ...
            <div className="space-y-2 max-h-[200px] overflow-y-auto">
              {ozelSablonForm.dersler.map((ders, idx) => (
                <div key={idx} className="flex gap-2 items-center bg-gray-50 p-2 rounded">
                  <input 
                    type="text" 
                    placeholder="Kod" 
                    className="w-16 px-2 py-1 border rounded text-xs focus:ring-1 focus:ring-amber-500"
                    value={ders.dersKodu}
                    onChange={(e) => {
                      const yeni = [...ozelSablonForm.dersler];
                      yeni[idx].dersKodu = e.target.value;
                      setOzelSablonForm({...ozelSablonForm, dersler: yeni});
                    }} 
                  />
                  <input 
                    type="text" 
                    placeholder="Ders Adı" 
                    className="flex-1 px-2 py-1 border rounded text-xs focus:ring-1 focus:ring-amber-500"
                    value={ders.dersAdi}
                    onChange={(e) => {
                      const yeni = [...ozelSablonForm.dersler];
                      yeni[idx].dersAdi = e.target.value;
                      setOzelSablonForm({...ozelSablonForm, dersler: yeni});
                    }} 
                  />
                  <input 
                    type="number" 
                    placeholder="Soru" 
                    className="w-14 px-2 py-1 border rounded text-xs focus:ring-1 focus:ring-amber-500"
                    value={ders.soruSayisi || ''}
                    onChange={(e) => {
                      const yeni = [...ozelSablonForm.dersler];
                      yeni[idx].soruSayisi = parseInt(e.target.value) || 0;
                      setOzelSablonForm({...ozelSablonForm, dersler: yeni});
                    }} 
                  />
                  <button 
                    className="p-1 text-red-500 hover:bg-red-50 rounded"
                    onClick={() => setOzelSablonForm({...ozelSablonForm, dersler: ozelSablonForm.dersler.filter((_, i) => i !== idx)})}>
                    <X size={14} />
                  </button>
                </div>
              ))}
              {ozelSablonForm.dersler.length === 0 && (
                <p className="text-xs text-gray-400 text-center py-4">Henüz ders eklenmedi</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="px-4 pb-4 flex justify-end">
          <button 
            className="px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 text-sm font-medium flex items-center gap-2"
            onClick={() => toast.success('Şablon kaydedildi!')}
          >
            <Save size={14} />
            Şablonu Kaydet
          </button>
