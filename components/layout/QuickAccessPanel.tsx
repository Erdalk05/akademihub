...
const QuickAccessPanel: React.FC = () => {
  ...
  const renderCard = (action: QuickAction, idx: number) => (
    <Link key={idx} href={action.href}>
      <div className="bg-white rounded-xl shadow-sm hover:shadow-lg border-2 border-[#25D366]/20 transition-all transform hover:scale-[1.02] hover:border-[#25D366] cursor-pointer p-5 h-full group flex flex-col items-center text-center">
        {/* İkon - Büyük ve Ortalı */}
        <div className={`bg-gradient-to-br ${action.gradient} w-16 h-16 rounded-2xl flex items-center justify-center text-white mb-4 shadow-lg group-hover:shadow-xl group-hover:scale-110 transition-all`}>
          {action.icon}
        </div>
        {/* Başlık - Ortalı */}
        <h3 className="font-bold text-[#075E54] text-sm group-hover:text-[#128C7E] transition-colors mb-1">{action.label}</h3>
        {/* Açıklama - Ortalı */}
